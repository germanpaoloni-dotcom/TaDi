...
const id = "boda-boho-desertica";

const sampleData = {
  novia: "Delfina", novio: "Tomás",
  fecha: "2027-04-17", horaCeremonia: "18:00", lugarCeremonia: "Bodega El Esteco, Cafayate",
  horaFiesta: "21:00", lugarFiesta: "Finca Las Nubes, Cafayate",
  direccionMapa: "https://maps.google.com/?q=Cafayate+Salta",
  mensaje: "Entre viñedos, cerros ocres y el cielo más grande que vieron, queremos celebrar el comienzo de nuestro camino juntos rodeados de las personas que más queremos.",
  dressCode: "Boho chic — tonos tierra, comodidad ante todo",
  alias: "delfina.tomas.boda",
...
// pétalo alargado y afilado (mismo recurso de "hojita" que el resto del
// catálogo boho, pero estiradas y puntudas para leerse como agave).
...
function render(data = {}) {
  ...
  const giftHTML = d.alias
    ...
    <h2>¿Un regalo?</h2>
    ${macrameDivider("#8f4527")}
    <p class="gift-text">Lo más lindo para nosotros es contar con ustedes, pero si quieren hacernos un presente, un aporte para nuestro próximo capítulo nos llena de alegría.</p>
...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  /* ---------- HERO ---------- */
  .hero{text-align:center;padding-top:clamp(50px,9vw,90px);padding-bottom:clamp(56px,10vw,100px);}
  ...
  .cover-photo{width:100%;max-width:520px;height:clamp(240px,44vw,400px);object-fit:cover;border-radius:70px 6px 70px 6px;box-shadow:0 14px 30px rgba(74,47,34,.22);}

  .names{font-size:clamp(2.3rem,7vw,3.6rem);margin:6px 0 4px;}
  .fecha-grande{font-family:'Cormorant',serif;font-style:italic;font-weight:500;font-size:clamp(1.4rem,4.5vw,2.1rem);letter-spacing:2px;margin:12px 0 4px;}
  ...
  .map-link{color:var(--terracota-dark);text-decoration:none;border-bottom:1px solid var(--desert-accent);padding-bottom:3px;font-size:.95rem;white-space:nowrap;}
  .map-link:hover{color:var(--terracota);}
  .dresscode-box{border:1px solid var(--desert-accent);border-radius:4px;padding:14px 26px;text-align:center;max-width:260px;}
  .dresscode-box .kicker{margin:0 0 6px;}
  .dresscode-box strong{font-family:'Marcellus',serif;font-weight:400;font-size:1.1rem;color:var(--terracota-dark);display:block;}

  .itinerary{max-width:480px;margin:24px auto 0;text-align:left;}
  .itin-row{display:flex;gap:18px;align-items:baseline;padding:16px 0;border-bottom:1px solid rgba(253,243,231,.35);flex-wrap:wrap;}
  .itin-row:last-child{border-bottom:none;}
  .itin-time{font-family:'Cormorant',serif;font-weight:500;font-size:1.1rem;flex:0 0 auto;min-width:74px;}
  .itin-row strong{font-family:'Marcellus',serif;font-size:1.05rem;font-weight:400;}
  .itin-row p{margin:2px 0 0;opacity:.9;font-size:.92rem;}
  ...
  .rsvp-sep{align-self:stretch;background:linear-gradient(var(--sand2) 0, var(--terracota) 12%, var(--terracota) 88%, var(--sand2) 100%);opacity:.5;}
  ...
  .alias-box{display:inline-block;margin-top:14px;border:1px dashed var(--terracota-dark);border-radius:10px;padding:10px 20px;font-family:'Marcellus',serif;letter-spacing:.5px;}
  ...
  .gallery img{width:100%;height:clamp(120px,22vw,190px);object-fit:cover;border-radius:34px 6px 34px 6px;cursor:pointer;box-shadow:0 8px 20px rgba(74,47,34,.25);}
  .lightbox{display:none;position:fixed;inset:0;background:rgba(40,22,14,.94);align-items:center;justify-content:center;z-index:50;}
  ...
  .rsvp-form label{font-size:.75rem;text-transform:uppercase;letter-spacing:1px;color:var(--terracota-dark);}
  .rsvp-form input,.rsvp-form select,.rsvp-form textarea{font-family:'Jost',sans-serif;padding:11px 12px;border:1px solid var(--rosa-dark);border-radius:10px;margin-top:5px;width:100%;background:var(--cream);color:var(--ink);}
  .rsvp-form button{background:var(--terracota-dark);color:#fdf3e7;border:0;padding:13px;border-radius:30px;letter-spacing:1px;text-transform:uppercase;font-size:.82rem;cursor:pointer;transition:background .2s;}
  .rsvp-form button:hover{background:var(--terracota);}
  .rsvp-whatsapp{text-align:center;color:var(--terracota-dark);font-size:.85rem;text-decoration:underline;}
  .rsvp-status{text-align:center;color:var(--terracota-dark);font-weight:500;}
...
<body>
  ...
  <div class="band band-sunset hero">
    ${desertSunSVG("sunGradHero", "#fbe0b8", accent)}
  ...
  ${(d.horaCeremonia || d.lugarCeremonia || d.horaFiesta || d.lugarFiesta) ? `<div class="band band-terracota">
    ${corners("#fbe3cd")}
    <div class="wrap">
...
function cardPreview(d) {
  return `<div style="position:absolute;inset:0;background:linear-gradient(180deg,#e8a87c 0%,#d9895f 45%,${d.accent} 100%);overflow:hidden;display:flex;align-items:center;justify-content:center;">
    <svg style="position:absolute;bottom:-4px;left:8px;width:56px;height:auto;opacity:.9;" viewBox="0 0 60 112" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    ...
    <svg style="position:absolute;bottom:-4px;right:8px;width:66px;height:auto;opacity:.85;transform:scaleX(-1);" viewBox="0 0 100 92" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    ...
    <svg style="position:absolute;top:10px;left:50%;transform:translateX(-50%);width:100px;height:100px;opacity:.4;" viewBox="0 0 220 220" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <circle cx="110" cy="110" r="100" fill="#fbe0b8"/>
    </svg>
    <div style="position:relative;z-index:1;text-align:center;padding:0 14px;">
      <div style="font-family:Georgia,'Times New Roman',serif;font-size:1.15rem;color:#fffaf3;letter-spacing:.5px;text-shadow:0 1px 4px rgba(74,47,34,.4);">${esc(d.name)}</div>
...
module.exports = {
  id, category: "bodas", name: "Bohemia Desértica",
  summary: "Terracota y rosa polvoriento al atardecer, cactus y agaves dibujados a mano con detalles de macramé — boho desértico para bodas a cielo abierto.",
  accent: "#c1623b", accent2: "#d9a0a0", schema: bodaSchema, sampleData, render, cardPreview,
};
